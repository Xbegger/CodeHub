...
class MyS7Client(Base):
    ...
    def __init__(self, name, ip, src_ip, port=102, src_tsap='\x01\x00', rack=0, slot=2, timeout=4):
        '''
    ...
    def connect(self):
        if(self._connection != None):
            ...
            self.logger.info("Close old socket!")
        self.logger.info("Start connect PLC")
        sock = socket.socket()
    ...
    def send_receive_packet(self, packet):
        if self._connection:
            try:
                rsp = self._connection.sr1(packet, timeout=self._timeout, retry=3)
                return rsp
    ...
    def send_s7_packet(self, packet):
        if self._connection:
            ...
            try:
                ...
                send(packet, socket=self._connection, realtime=True, verbose=False)
    ...
    '''
    def onlinePLC(self, timeout = 1, retry=1):
        ...
        # packet = ARP(op=1, pdst=self._ip, psrc=self._src_ip)
        # recv = sr1(packet, retry=retry, timeout=timeout)

        # pkt = ARP(op=1, pdst=self._ip, psrc=self._src_ip)
        # pkt = IP(dst="192.168.1.188", src="192.168.1.137") / ICMP()
        # res = sr1(pkt, retry=retry, timeout=timeout)
        # # recv = srp1flood(packet)
        if(recv != None):
    ...
    def crushListen(self):
        ...
        sniff(filter=filter, prn=self.findArpPacket)
        # self._connection.sniff(prn= lambda x: self.logger.info(raw(x).hex()))

    def findArpPacket(self, pkt):
        self.crush.set()
        self.logger.info("passive detected a probable crush " + raw(pkt).hex())
