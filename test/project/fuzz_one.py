...
packet_unsearched = db.unsearched
...
error_packet = []
packets = [pkt for i in range(1000)]
randlist = []
head = 0
while (head < 1000):
  rand = random.randint(head, head+100)
...
for packet in packets:
  ...
  if arp_capture_thread.get_result() == False:
    ...
    if target.onlinePLC() == False:
      for pp in packet_set:
        pp_str = raw(pp).hex()
        packet_unsearched.insert_one({'packet': pp_str})
      plug.on()
      time.sleep(20)
      target.connect()
    else:
